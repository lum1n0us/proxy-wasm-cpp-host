...
bool Wamr::load(const std::string &code, bool allow_precompiled) {
  ...
  wasm_limits_t limits = {10, 30};
  // wasm_table_new(store_.get(), wasm_tabletype_new(wasm_valtype_new(WASM_FUNCREF), &limits), nullptr);
...
bool Wamr::link(std::string_view debug_name) {
  ...
  /* TODO: verify: link by WAMR automatically */

  instance_ = wasm_instance_new(store_.get(), module_.get(), nullptr, nullptr);
